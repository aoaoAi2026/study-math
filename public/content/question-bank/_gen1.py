# -*- coding: utf-8 -*-
"""生成低年级综合题库 (6 files, 15 questions each)"""
import json, os
...
qs = []
qs.append(q("cm-cg-01","fill",1,"按规律填数：2、4、6、8、____",
             "10",["10","十"],
             solution=["这是连续的双数(偶数),每次加2","8+2=10"],
             hints=["每次多几?","都是双数"],
             mistakes=[{"mistake":"写成9","reason":"以为每次加1","correction":"每次加2,8+2=10","errorLayer":"L1"}],
             kp=["偶数","数列规律"], t=45, tags=["找规律"]))
qs.append(q("cm-cg-02","fill",1,"按规律填数：10、15、20、25、____",
             "30",["30","三十"],
             solution=["每次加5,都是5的倍数","25+5=30"],
             hints=["每次多5","数一数:10,15,20,25,..."],
             mistakes=[{"mistake":"写成35","reason":"加错了","correction":"25+5=30","errorLayer":"L1"}],
             kp=["倍数","数列规律"], t=45, tags=["找规律"]))
qs.append(q("cm-cg-03","fill",1,"按规律填数：1、2、4、8、____",
             "16",["16","十六"],
             solution=["后一个数是前一个数的2倍","8×2=16"],
             hints=["每个数是前面的几倍?","1×2=2,2×2=4..."],
             mistakes=[{"mistake":"写成12","reason":"以为每次加4","correction":"8×2=16","errorLayer":"L1"}],
             kp=["等比数列"], t=45, tags=["找规律"]))
qs.append(q("cm-cg-04","fill",2,"按规律填数：1、4、9、16、____、36",
             "25",["25","二十五"],
             solution=["每个数是位置序号的平方","1×1=1,2×2=4,3×3=9,4×4=16,5×5=25"],
...
qs.append(q("cm-cg-05","fill",2,"找规律填数：2、6、12、20、____、42",
             "30",["30","三十"],
             solution=["相邻两数的差是:4,6,8,10,12","20+10=30","或1×2=2,2×3=6,3×4=12,4×5=20,5×6=30"],
             hints=["算一算差","差:4,6,8,...","也可写成n×(n+1)"],
             mistakes=[{"mistake":"写成28","reason":"差算成8","correction":"差是10,20+10=30","errorLayer":"L2"}],
             kp=["差数列"], t=60, tags=["找规律"]))
qs.append(q("cm-cg-06","fill",2,"找规律填数：1、1、2、3、5、8、____、21",
             "13",["13","十三"],
             solution=["每个数等于前面两个数的和","5+8=13","这是斐波那契数列"],
...
qs.append(q("cm-cg-07","choice",2,"一串珠子按白、黑、白、黑、白、黑排列,第20颗是什么颜色?",
             "1",None,options=["白色","黑色","红色","无法确定"],
...
qs.append(q("cm-cg-08","fill",3,"找规律：1、3、6、10、15、____",
             "21",["21","二十一"],
...
qs.append(q("cm-cg-09","fill",3,"小明从1写到100,一共写了多少个数字1?",
             "21",["21","二十一"],
             solution=["个位上是1的数:1,11,21,...,91,共10个","十位上是1的数:10,11,12,...,19,共10个",
                       "百位上是1的数:100,1个","合计:10+10+1=21个"],
...
qs.append(q("cm-cg-10","fill",3,"一串数:1、2、2、3、3、3、4、4、4、4、...第50个数是多少?",
             "10",["10","十"],
             solution=["数字n出现n次","前9个数累计:1+2+3+...+9=45","前10个数累计:45+10=55","第46到55个都是10","第50个是10"],
             hints=["先算前n个数共有多少个","1+2+3+...+n=n(n+1)/2","找到50在哪个范围内"],
             mistakes=[{"mistake":"写成9","reason":"算错累计数","correction":"前9个共45,第50个是10","errorLayer":"L2"}],
             kp=["数列","高斯求和"], t=100, tags=["计数"]))
qs.append(q("cm-cg-11","fill",3,"把1、2、3、4、5、6、7、8、9这九个数字填入□里,每个数字只用一次,使等式成立:□+□=□+□=□+□=□+□=□。问中间单独的数是多少?",
             "5",["5","五"],
             solution=["1到9的和是45","设单独的数为a,剩下8个数之和=45-a,要能分成4对相等的和","45-a必须能被4整除","a=5时:45-5=40,40÷4=10","验证:1+9=2+8=3+7=4+6=10,余5✓"],
...
qs.append(q("cm-cg-12","fill",4,"在1、2、3、4、5之间添上+或-,使结果等于5。\n1□2□3□4□5=5\n写出一种符号顺序(用+-表示,如+++-)",
             "+++-",["+++-","-++-","+-+-+"],
             solution=["全部加起来是15,要让结果为5,需减少10","把某些+改成-,每改一个,总和减少该数的2倍","1+2+3+4-5=5 ✓,答案+++-","或-1+2+3-4+5=5"],
             hints=["全加=15","每把一个+改成-,总和减少该数的2倍","需要减少10,所以要把和为5的数变成减号"],
             mistakes=[{"mistake":"随便乱写","reason":"没有方法","correction":"答案+++-或-++-等","errorLayer":"L3"}],
             kp=["巧填符号","凑数"], t=150, tags=["填符号"]))
qs.append(q("cm-cg-13","fill",4,"一列数:1、2、3、2、3、4、3、4、5、4、5、6、...前100个数的和是多少?",
             "1815",["1815"],
             solution=["观察:可以每3个一组(1,2,3),(2,3,4),(3,4,5),...每组和是6,9,12,...构成等差数列","前99个数是33组,第n组的和=3n+3(n+1)?实际每组和是:6,9,12,...,即首项6公差3的等差数列","重新观察:第1组(1,2,3)=6,第2组(2,3,4)=9,第33组(33,34,35)=102","33组的和=(6+102)×33÷2=108×33÷2=1782","第100个数=34","总和=1782+33=1815"],
             hints=["每3个一组看看","每组的第一个数是1,2,3,...","每组和:6,9,12,...是等差数列"],
             mistakes=[{"mistake":"写成1782","reason":"忘了加第100个数","correction":"前99个=1782,第100个=33,共1815","errorLayer":"L3"}],
             kp=["数列分组","等差数列求和"], t=180, tags=["分组求和"]))
qs.append(q("cm-cg-14","fill",4,"一个数加上3,再乘3,再减3,再除以3,结果还是3。这个数是多少?",
             "1",["1","一"],
...
qs.append(q("cm-cg-15","fill",5,"有一个三位数,十位上的数字是百位的2倍,个位上的数字是十位的2倍。这个数可能是多少?(写出最小的那个)",
             "124",["124"],
             solution=["设百位数字为a,则十位=2a,个位=4a","每个数字≤9,百位a≥1","当a=1时:百=1,十=2,个=4,数=124✓","当a=2时:百=2,十=4,个=8,数=248✓","最小的数是124"],
...
qs=[]
qs.append(q("cm-ad-01","fill",1,"5 + 3 = ?","8",["8","八"],
             solution=["5+3=8"],hints=["数手指:5,6,7,8"],
             mistakes=[{"mistake":"写成7","reason":"数错","correction":"5+3=8","errorLayer":"L1"}],
             kp=["加法"],t=30,tags=["加法"]))
qs.append(q("cm-ad-02","fill",1,"9 - 4 = ?","5",["5","五"],
             solution=["9-4=5"],hints=["从9倒数4个数"],
             mistakes=[{"mistake":"写成6","reason":"数错","correction":"9-4=5","errorLayer":"L1"}],
             kp=["减法"],t=30,tags=["减法"]))
qs.append(q("cm-ad-03","fill",1,"7 + 6 = ?","13",["13","十三"],
             solution=["7+6=13。凑十法:7+3=10,10+3=13"],
             hints=["7+3=10,再加3"],
             mistakes=[{"mistake":"写成12","reason":"凑十法算错","correction":"7+3+3=13","errorLayer":"L1"}],
             kp=["进位加法","凑十法"],t=40,tags=["凑十法"]))
qs.append(q("cm-ad-04","fill",2,"15 - 7 = ?","8",["8","八"],
             solution=["15-7=8。破十法:10-7=3,3+5=8"],
             hints=["10-7=3,3+5=8","想7+?=15"],
             mistakes=[{"mistake":"写成7","reason":"退位错误","correction":"15-7=8","errorLayer":"L2"}],
             kp=["退位减法","破十法"],t=45,tags=["退位减法"]))
qs.append(q("cm-ad-05","fill",2,"8 + 5 + 2 = ?","15",["15","十五"],
             solution=["8+5+2=15。先算8+2=10凑十,再加5=15"],
             hints=["先凑十:8+2=10"],
             mistakes=[{"mistake":"写成14","reason":"计算失误","correction":"(8+2)+5=15","errorLayer":"L2"}],
             kp=["连加","巧算"],t=45,tags=["连加"]))
qs.append(q("cm-ad-06","fill",2,"17 - 5 - 2 = ?","10",["10","十"],
             solution=["17-5-2=10","或17-(5+2)=10"],
             hints=["也可以先算5+2=7","17-7=10"],
             mistakes=[{"mistake":"写成11","reason":"17-5算成12","correction":"17-7=10","errorLayer":"L2"}],
             kp=["连减"],t=45,tags=["连减"]))
qs.append(q("cm-ad-07","fill",2,"在□里填数:7 + □ = 13","6",["6","六"],
             solution=["□=13-7=6","验证:7+6=13✓"],
             hints=["13-7=?","想7加多少等于13"],
             mistakes=[{"mistake":"写成5","reason":"减法算错","correction":"13-7=6","errorLayer":"L2"}],
...
qs.append(q("cm-ad-09","fill",3,"一本书共20页,小华第一天看了5页,第二天看了7页。还剩多少页没看?",
             "8",["8","八"],
             solution=["20-5-7=8页","或20-(5+7)=8页"],
             hints=["总页数-第一天-第二天","也可以先算两天一共看了多少"],
             mistakes=[{"mistake":"写成9","reason":"20-12算错","correction":"20-12=8","errorLayer":"L2"}],
             kp=["连减应用题"],t=60,tags=["应用题"]))
qs.append(q("cm-ad-10","fill",3,"哥哥有9颗糖,比弟弟多3颗。弟弟有几颗糖?",
             "6",["6","六"],
             solution=["哥哥比弟弟多3颗,所以弟弟比哥哥少3颗","弟弟=9-3=6颗"],
             hints=["比...多=反方向比...少","哥=弟+3,所以弟=哥-3"],
             mistakes=[{"mistake":"写成12","reason":"看到'多'就加","correction":"弟弟=9-3=6","errorLayer":"L2"}],
             kp=["比多比少","应用题"],t=60,tags=["比多比少"]))
qs.append(q("cm-ad-11","fill",3,"小明从家到学校要走15分钟。他要在8点到校,最晚几点几分从家出发?",
             "7:45",["7:45","7点45","7点45分","7时45分"],
             solution=["8时-15分=7时45分"],
             hints=["用到校时间减走路时间","8:00-15分钟"],
             mistakes=[{"mistake":"写成8:15","reason":"方向搞反","correction":"8:00-15分=7:45","errorLayer":"L2"}],
             kp=["时间计算"],t=70,tags=["时间"]))
qs.append(q("cm-ad-12","fill",4,"□ + △ = 12,△ - 3 = 5,问□ = ?△ = ?(回答□的值)",
             "4",["4","四"],
...
qs.append(q("cm-ad-13","fill",4,"一桶油连桶重15千克,倒出一半油后连桶重8千克。原来油重多少千克?(回答油重)",
             "14",["14","十四"],
...
qs.append(q("cm-ad-14","fill",4,"一排小朋友从左数小明排第5,从右数小明排第6。这一排共多少人?",
             "10",["10","十"],
             solution=["5+6-1=10人","小明被数了两次,要减1"],
             hints=["画个图","小明被数了两次"],
             mistakes=[{"mistake":"写成11","reason":"忘了减1","correction":"5+6-1=10","errorLayer":"L2"}],
             kp=["排队问题"],t=80,tags=["排队"]))
qs.append(q("cm-ad-15","fill",5,"小明、小红、小刚三人共有糖18颗。小明给小红2颗后,小红再给小刚3颗,小刚再给小明1颗,这时三人糖数相等。问小明原来有多少颗糖?",
             "7",["7","七"],
             solution=["最后三人相等,各有18÷3=6颗","用倒推法求小明:小明最后6颗,之前他收到小刚给的1颗,所以之前是5颗","再往前,小明曾给小红2颗,所以原来=5+2=7颗"],
...
qs.append(q("cm-wp-06","fill",2,"红花有8朵,黄花比红花多5朵。黄花有多少朵?",
             ...
             hints=["比...多→加"],
             mistakes=[{"mistake":"写成11","reason":"8+5算错","correction":"8+5=13","errorLayer":"L2"}],
...
qs.append(q("cm-wp-14","fill",4,"3只鸡3天下3个蛋,9只鸡9天下几个蛋?",
             "27",["27","二十七"],solution=["3只鸡3天→3个蛋→1只鸡1天→1/3个蛋","9只鸡9天:9×9×(1/3)=27个蛋","或:鸡数×3,天数×3,蛋数×9倍。3×9=27个"],
...
qs.append(q("cm-wp-15","fill",5,"小明、小红、小刚三人共有糖18颗。小明给小红2颗后,小红再给小刚3颗,小刚再给小明1颗,这时三人糖数相等。问小明原来有多少颗糖?",
             "7",["7","七"],solution=["最后三人相等,各有18÷3=6颗","用倒推法求小明:最后6颗,之前收到小刚给的1颗,所以之前是5颗","再往前,小明曾给小红2颗,所以原来=5+2=7颗"],
             hints=["最后每人6颗","用倒推:给出去的加回来,收到的减回去","小明:6-1+2=7"],
             mistakes=[{"mistake":"写成6","reason":"没倒推","correction":"倒推:6-1+2=7","errorLayer":"L3"}],
             kp=["还原问题","倒推法"],t=180,tags=["倒推","还原"]))
...
qs=[]
qs.append(q("cm-mt-01","fill",1,"6 × 7 = ?","42",["42","四十二"],
             solution=["六七四十二,所以6×7=42"],
             hints=["六六三十六,加6就是六七"],
             mistakes=[{"mistake":"写成48","reason":"和六八搞混","correction":"六七四十二","errorLayer":"L1"}],
             kp=["乘法口诀"],t=30,tags=["乘法"]))
qs.append(q("cm-mt-02","fill",1,"8 × 9 = ?","72",["72","七十二"],
             solution=["八九七十二"],
             hints=["九九八十一减9"],
             mistakes=[{"mistake":"写成63","reason":"七九搞混","correction":"八九七十二","errorLayer":"L1"}],
             kp=["乘法口诀"],t=30,tags=["乘法"]))
qs.append(q("cm-mt-03","fill",1,"7 × 8 = ?","56",["56","五十六"],
             solution=["七八五十六"],
             hints=["7×7+7=56"],
             mistakes=[{"mistake":"写成54","reason":"和六九搞混","correction":"七八五十六","errorLayer":"L1"}],
             kp=["乘法口诀"],t=30,tags=["乘法"]))
qs.append(q("cm-mt-04","fill",2,"□ × 8 = 56,□ = ?","7",["7","七"],
             solution=["想乘法口诀:七八五十六,所以□=7","或56÷8=7"],
             hints=["56÷8=?","想乘法口诀"],
             mistakes=[{"mistake":"写成6","reason":"六八四十八","correction":"七八五十六,填7","errorLayer":"L2"}],
             kp=["乘法填空"],t=40,tags=["填空"]))
qs.append(q("cm-mt-05","fill",2,"9 × 9 + 9 = ?","90",["90","九十"],
             solution=["9×9=81,81+9=90","或看成10个9相加=90"],
             hints=["9个9加1个9=10个9"],
             mistakes=[{"mistake":"写成89","reason":"81+9算错","correction":"81+9=90","errorLayer":"L2"}],
             kp=["乘加混合"],t=45,tags=["混合运算"]))
qs.append(q("cm-mt-06","fill",2,"36 ÷ 6 × 2 = ?","12",["12","十二"],
...
qs.append(q("cm-mt-08","fill",3,"一个星期有7天。5个星期有多少天?",
             ...
             hints=["7×5=?"],
             mistakes=[{"mistake":"写成28","reason":"四七二十八","correction":"五七三十五","errorLayer":"L2"}],
...
qs.append(q("cm-mt-11","fill",3,"3只鸡3天下3个蛋,9只鸡9天下几个蛋?",
             "27",["27","二十七"],solution=["3只鸡3天→3个蛋→1只鸡3天→1个蛋→1只鸡1天→1/3个蛋","9只鸡9天:9×9×(1/3)=27个蛋","或:鸡数×3,天数×3,蛋数×9倍。3×9=27个"],
             hints=["先求1只鸡1天下几个","别被绕晕了!","鸡和天数各放大3倍,蛋×9倍"],
             mistakes=[{"mistake":"写成9","reason":"简单放大3倍","correction":"鸡和天数各×3,蛋×9=27","errorLayer":"L3"}],
             kp=["归一问题"],t=90,tags=["归一"]))
qs.append(q("cm-mt-12","fill",4,"计算:1+2+3+4+5+6+7+8+9 = ?","45",["45","四十五"],
             solution=["配对:(1+9)+(2+8)+(3+7)+(4+6)+5=10+10+10+10+5=45","或9×5=45(中间数5乘9个数)"],
...
qs.append(q("cm-mt-14","fill",4,"一个两位数,十位上的数字是个位的2倍,两个数字的和是9。这个两位数是多少?",
             "63",["63"],
             solution=["设个位数字为a,十位=2a","2a+a=9,3a=9,a=3","个位=3,十位=6,数=63"],
             hints=["设个位为a,十位=2a","两数字和=3a=9"],
             mistakes=[{"mistake":"写成36","reason":"十位个位搞反","correction":"十位=6,个位=3,所以是63","errorLayer":"L3"}],
             kp=["位值原理","数字问题"],t=120,tags=["数字"]))
qs.append(q("cm-mt-15","fill",5,"一个自然数与它自己相加、相减、相乘、相除所得的和、差、积、商加起来,恰好是100。这个自然数是多少?",
             "9",["9","九"],
             solution=["设这个数为x","相加:x+x=2x,相减:x-x=0,相乘:x×x=x²,相除:x÷x=1","总和:x²+2x+0+1=(x+1)²=100","x+1=10,x=9","验证:18+0+81+1=100✓"],
...
qs.append(q("cm-fa-03","fill",1,"3/7 + 4/7 = ?","1",["1","7/7"],
...
qs.append(q("cm-fa-05","fill",2,"5/6 - 1/3 = ?","1/2",["1/2","二分之一","3/6"],
             ...
             mistakes=[{"mistake":"写成4/3","reason":"没通分","correction":"5/6-2/6=3/6=1/2","errorLayer":"L2"}],
             kp=["异分母减法","约分"],t=70,tags=["异分母"]))
qs.append(q("cm-fa-06","fill",2,"2 + 3/4 = ?(用带分数回答)","2 3/4",["2 3/4","2又3/4","11/4"],
             solution=["整数加真分数直接写成带分数:2+3/4=2又3/4","或2=8/4,8/4+3/4=11/4"],
             hints=["整数+真分数=带分数"],
...
qs.append(q("cm-fa-09","fill",3,"1/2 + 1/4 + 1/8 + 1/16 = ?","15/16",["15/16"],
             solution=["通分到16:8/16+4/16+2/16+1/16=15/16","或用1减剩余:原式=1-1/16=15/16"],
             hints=["画图:正方形一半+1/4+1/8+1/16","剩下1/16,所以=1-1/16"],
             mistakes=[{"mistake":"写成7/8","reason":"加到1/8就停了","correction":"加1/16=15/16","errorLayer":"L2"}],
             kp=["连加","裂项"],t=100,tags=["分数连加"]))
qs.append(q("cm-fa-10","fill",3,"一堆煤,第一天用去1/3,第二天用去1/4,还剩几分之几?",
             "5/12",["5/12"],
             solution=["把总量看作1。剩下=1-1/3-1/4","=12/12-4/12-3/12=5/12"],
...
qs.append(q("cm-fa-11","fill",3,"一项工程甲独做10天完成,乙独做15天完成。两人合做几天完成?",
             "6",["6","六"],
             solution=["甲每天做1/10,乙每天做1/15","两人一天共做:1/10+1/15=3/30+2/30=5/30=1/6","1÷1/6=6天"],
             hints=["把总工程看作1","甲效率1/10,乙1/15","合做效率=1/10+1/15=1/6"],
             mistakes=[{"mistake":"选(10+15)/2","reason":"取平均","correction":"1÷(1/10+1/15)=6天","errorLayer":"L3"}],
             kp=["工程问题"],t=120,tags=["工程问题"]))
qs.append(q("cm-fa-12","fill",4,"1/6 + 1/12 + 1/20 + 1/30 + 1/42 = ?","5/14",["5/14"],
             solution=["注意:1/6=1/(2×3)=1/2-1/3,1/12=1/(3×4)=1/3-1/4...","=(1/2-1/3)+(1/3-1/4)+(1/4-1/5)+(1/5-1/6)+(1/6-1/7)","=1/2-1/7=5/14"],
...
qs.append(q("cm-fa-13","fill",4,"一桶水,第一次倒出1/2,第二次倒出剩下的1/3,第三次倒出剩下的1/4,还剩5升。原来有多少升?",
             "20",["20","二十"],
             solution=["倒推法。设原来x升","第一次后剩x×(1-1/2)=x/2","第二次后剩x×(1-1/2)×(1-1/3)=x×1/2×2/3","第三次倒出剩的1/4,剩x×1/2×2/3×3/4=x/4","答案=x/4=5,所以x=20","或直接算:剩余=x×(1-1/2)×(1-1/3)×(1-1/4)=x×1/2×2/3×3/4=x/4=5,x=20升"],
...
qs.append(q("cm-fa-14","fill",4,"2/3 + 3/4 + 4/5 + 5/6 = ?(用最简分数)","283/60",["283/60","4 43/60"],
             solution=["通分,分母最小公倍数=60","2/3=40/60,3/4=45/60,4/5=48/60,5/6=50/60","总和=(40+45+48+50)/60=183/60=61/20=3 1/20","重新计算:2/3=40/60,3/4=45/60,4/5=48/60,5/6=50/60,合计=183/60=61/20"],
             hints=["通分到60","2/3=40/60,3/4=45/60,4/5=48/60,5/6=50/60"],
             mistakes=[{"mistake":"写成283/60","reason":"计算错","correction":"=183/60=61/20=3又1/20","errorLayer":"L3"}],
             kp=["异分母加法","通分"],t=150,tags=["通分"]))
qs.append(q("cm-fa-15","fill",5,"计算:1/2 + 5/6 + 11/12 + 19/20 + 29/30 + 41/42 + 55/56 + 71/72 = ?(用最简分数)",
             "7 2/9",["7 2/9","65/9","7又2/9"],
             solution=["注意到每项可以写成1-1/n(n+1):","1/2=1-1/2,5/6=1-1/6,11/12=1-1/12...","原式=8-(1/2+1/6+1/12+1/20+1/30+1/42+1/56+1/72)","=8-(1/1×2+1/2×3+1/3×4+...+1/8×9)","=8-(1-1/2+1/2-1/3+...+1/8-1/9)","=8-(1-1/9)=8-8/9=72/9-8/9=64/9=7又1/9"],
             hints=["每项=1-1/(n(n+1))","1/2=1-1/2,5/6=1-1/6,11/12=1-1/12...","用裂项:1/1×2+1/2×3+...=1-1/9"],
             mistakes=[{"mistake":"写成7 1/9","reason":"裂项算错","correction":"8-(1-1/9)=7+1/9=7又1/9...实际=8-8/9=64/9=7又1/9","errorLayer":"L3"}],
...
qs.append(q("cm-eq-05","fill",2,"3x - 7 = 8,x = ?","5",["5","五"],
             solution=["3x=8+7=15","x=15÷3=5"],
             hints=["先移项:3x=8+7","再除以3"],
...
qs.append(q("cm-eq-07","fill",2,"x/4 + 3 = 7,x = ?","16",["16","十六"],
             solution=["x/4=7-3=4","x=4×4=16"],
             hints=["先移项:x/4=7-3","再乘以4"],
...
qs.append(q("cm-eq-10","fill",3,"小明买了5本练习本和一支2元的铅笔,一共花了17元。每本练习本多少元?",
             "3",["3","三"],
             solution=["设每本练习本x元","5x+2=17","5x=15,x=3元"],
...
qs.append(q("cm-eq-11","fill",3,"一个两位数,十位数字是个位数字的2倍,如果把十位数字与个位数字对调,得到的新数比原数小27。求原两位数。",
             "63",["63"],
             solution=["设个位数字为x,则十位数字为2x","原数=10×2x+x=21x","对调后新数=10×x+2x=12x","21x-12x=27,9x=27,x=3","原数=63","验证:63-36=27✓"],
             hints=["设个位为x,十位为2x","原数=10×2x+x=21x","新数=10x+2x=12x","21x-12x=27"],
...
qs.append(q("cm-eq-12","fill",4,"小明的储蓄罐里有1角和5角的硬币共35枚,总值11元5角。问1角硬币有多少枚?",
             "15",["15","十五"],
             solution=["设1角硬币有x枚,则5角硬币有(35-x)枚","总值:1×x+5×(35-x)=115角","x+175-5x=115","-4x=-60,x=15","所以1角硬币有15枚,5角硬币有20枚"],
...
qs.append(q("cm-eq-13","fill",4,"父亲今年32岁,儿子今年8岁。多少年后父亲的年龄是儿子的3倍?",
             "4",["4","四"],
             solution=["设x年后父亲年龄是儿子的3倍","32+x=3×(8+x)","32+x=24+3x","32-24=3x-x","8=2x,x=4"],
             hints=["